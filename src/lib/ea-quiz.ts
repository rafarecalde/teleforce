...
export function bandCopy(answers: QuizAnswers, band: Band): BandCopy {
  ...
  if (band === 'burned') {
    return {
      ...
      next: 'Priority match. We match, onboard, and stay with you with full back-end support. If the fit fails, we rematch.',
  ...
  if (band === 'ready') {
    return {
      ...
      next: 'Book a match. Top-tier bilingual EN/ES, LatAm nearshore, U.S. hours. Match, onboard, and go live — backed by Teleforce.',
      cta: 'Book your match',
      silent: false,
    };
  }
  ...
  const own =
    answers.ownership === 'no'
      ? {
          ...
          ea: '2–4 weeks of investment. We match and onboard, then you go live with full back-end support.',
        }
      : {
          label: 'Ownership',
